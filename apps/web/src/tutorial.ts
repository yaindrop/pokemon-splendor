 ...
 * ====================================================================================== */
import type { BaseTier, Card, GameState, Player, TokenCounts } from '@pokemon-splendor/game-core';
...
(function () {
  ...
  interface TutorialStep {
    ...
    readonly detect?: (game: GameState, context: TutorialSnapshot) => boolean;
    readonly bodyClass?: string;
  }

  function player(game: GameState): Player {
    const first = game.players[0];
    if (!first) throw new Error('教程缺少玩家');
    return first;
  }

  function card(id: string): Card {
    const value = byId[id];
    if (!value) throw new Error(`教程卡牌不存在：${id}`);
    return value;
  }

  let active = false;
  let curMode: TutorialMode = 'base';
  let steps: readonly TutorialStep[] = [];
  let idx = 0;
  let ctx: TutorialSnapshot | null = null;

  // ---------------------------------------------------------------- scenario builders
  function purge(g: GameState, id: string): void {
    // remove an id from every deck and field slot
  ...
  function placeField(g: GameState, tier: BaseTier, slot: number, id: string): void {
    purge(g, id);
    g.field[tier][slot] = id;
  }
  function give(g: GameState, id: string): void {
    purge(g, id);
    player(g).board.push(id);
  }
  function setTokens(g: GameState, tokens: Partial<TokenCounts>): void {
    const p = player(g);
    p.tokens = { red: 0, blue: 0, black: 0, pink: 0, yellow: 0, purple: 0 };
    for (const color of E.ALL_TOKENS) p.tokens[color] = tokens[color] ?? 0;
  }
  const tokenTotal = (g: GameState): number => E.tokenTotal(player(g));
  const score = (g: GameState): number => E.scoreOf(g, player(g));

  function buildBase(): GameState {
    const g = E.createGame(PS.DB, { numPlayers: 1, names: ['你'], ai: [false], winScore: 999 });
    give(g, 's1_14');
    give(g, 's1_20'); // 2× 凯西 (pink) → 2 pink discounts + 2 VP
  ...
  function buildMega(): GameState {
    const g = E.createGame(PS.DB, {
      numPlayers: 1,
      names: ['你'],
      ai: [false],
      megas: true,
  ...
  // ---------------------------------------------------------------- steps
  const baseSteps: readonly TutorialStep[] = [
    {
      title: '欢迎来到训练家学院 🎓',
    ...
    },
    {
      ...
      arrange: (g) => {
        for (const c of COLORS) g.supply[c] = 4;
        setTokens(g, {});
      },
      target: '#supply',
    ...
    },
    {
      ...
      arrange: (g) => {
        setTokens(g, { red: 2 });
        placeField(g, 'stage1', 1, 's1_04');
        E.refill(g, 'stage1');
      },
      target: () => document.querySelector('.card[data-card="s1_04"]'),
      detect: (g, c) => player(g).board.length > c.board,
    },
    {
      title: '④ 进化',
      ...
      detect: (g, c) => player(g).buried.length > c.buried,
    },
    {
      title: '⑤ 预留宝可梦',
      ...
      detect: (g, c) => player(g).reserve.length > c.reserve,
    },
    {
      title: '⑥ 拿下胜利！🏆',
      ...
      arrange: (g) => {
        ...
        for (const c of COLORS) t[c] = Math.max(0, target.cost[c] - b[c]);
        p.tokens = t;
        g.winScore = score(g) + target.vp; // this single capture wins exactly
      },
  ...
  const megaSteps: readonly TutorialStep[] = [
    {
      title: '超级进化扩展 ⚡',
    ...
    },
    {
      ...
      detect: (g) => player(g).board.some((id) => card(id).tier === 'mega'),
    },
  ];
  ...
  function requireElement(id: string): HTMLElement {
    ...
    if (!element) throw new Error(`教程界面元素不存在：${id}`);
    return element;
  }
  ...
  function ensureDOM(): void {
    ...
    bubble.innerHTML =
      '<div id="tut-step"></div><div id="tut-title"></div><div id="tut-text"></div><div id="tut-actions"></div>';
    document.body.appendChild(mask);
    document.body.appendChild(bubble);
  }
  const resolveTarget = (target: TutorialStep['target']): HTMLElement | null =>
  ...
  function positionSpot(): void {
    const s = steps[idx],
      mask = document.getElementById('tut-mask');
    if (!mask) return;
    const t = s ? resolveTarget(s.target) : null;
    if (!t) {
  ...
  function snapshot(g: GameState): TutorialSnapshot {
    const p = player(g);
    return {
      board: p.board.length,
      buried: p.buried.length,
  ...
  function showStep(): void {
    ...
    requireElement('tut-step').textContent = '第 ' + (idx + 1) + ' / ' + steps.length + ' 步';
    requireElement('tut-title').innerHTML = s.title;
    requireElement('tut-text').innerHTML = s.html;
    const acts = requireElement('tut-actions');
    acts.innerHTML = '';
    if (s.next) {
      ...
      hint.textContent = '按上面的提示操作…';
      acts.appendChild(hint);
      const sk = document.createElement('button');
      sk.className = 'ghost small';
      sk.textContent = '跳过此步';
      sk.onclick = next;
      acts.appendChild(sk);
    }
    const ex = document.createElement('button');
    ex.className = 'ghost small';
    ex.textContent = '退出教程';
    ex.onclick = exit;
    acts.appendChild(ex);
    requireElement('tut-bubble').classList.remove('hidden');
    const t = resolveTarget(s.target);
    if (t) {
      try {
        t.scrollIntoView({ block: 'center', inline: 'center' });
      } catch {
        /* Browser may reject scroll options. */
      }
    }
    positionSpot();
  }
  ...
  function finish(): void {
    ...
    removeListeners();
    document.body.classList.remove('tut-hide-endturn');
    const m = document.getElementById('tut-mask');
    if (m) m.classList.add('hidden');
    const wm = document.getElementById('win-modal');
    if (wm) wm.classList.add('hidden');
    const b = document.getElementById('tut-bubble');
    if (!b) return;
    requireElement('tut-step').textContent = '教程完成';
    requireElement('tut-title').innerHTML =
      curMode === 'megas' ? '⚡ 学会超级进化！' : '🏆 恭喜通关！';
    requireElement('tut-text').innerHTML =
      curMode === 'megas'
    ...
    go.textContent = '开始一局对局';
    go.onclick = exit;
    acts.appendChild(go);
    if (curMode !== 'megas') {
      const ag = document.createElement('button');
      ag.className = 'ghost small';
      ag.textContent = '再练一次';
      ag.onclick = () => {
        start('base');
      };
  ...
  function onRender(g: GameState | null): void {
    if (!active || !g) return;
    positionSpot();
    const s = steps[idx];
    if (s?.detect) {
      try {
        if (s.detect(g, ctx ?? snapshot(g))) next();
      } catch {
        /* A transient render may precede its target. */
      }
    }
  }
  ...
  function start(mode: TutorialMode): void {
    stop();
    ensureDOM();
    curMode = mode === 'megas' ? 'megas' : 'base';
    if (curMode === 'megas' && (!PS.MEGA_DB || !PS.MEGA_DB.length)) {
      // mega tutorial needs the Megas data loaded
