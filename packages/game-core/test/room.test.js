...
import { Engine as E, Room as RoomModule } from '@pokemon-splendor/game-core';
...
function makeRoom() {
  ...
  const room = new Room({
    ...
    maxSeats: 4,
    send: (cid, msg) => {
      (inbox[cid] = inbox[cid] || []).push(msg);
    },
  });
  const last = (cid, t) => {
    const a = (inbox[cid] || []).filter((m) => m.t === t);
    return a[a.length - 1];
  };
  const clear = () => {
    for (const k in inbox) inbox[k] = [];
  };
...
test('only host (seat 0) can start; start deals a redacted state per seat', () => {
  ...
  assert.ok(
    sA.state.decks.stage1.every((x) => x === null),
    'deck order hidden in broadcast',
  );
});
...
test('reserve stays hidden from opponent; visible to its owner', () => {
  ...
  room.onMessage('cA', {
    t: 'action',
    seq: 1,
    action: { type: 'reserve', target: { fromDeck: 'stage1' } },
  });
...
test('snapshot/restore round-trips the game (DO persistence across eviction)', () => {
  ...
  const room2 = new Room({
    cardDB: DB,
    send: (cid, msg) => {
      (inbox2[cid] = inbox2[cid] || []).push(msg);
    },
  });
  room2.restore(snap);
...
test('FIX1: a spectator (no seat) gets a fully-redacted view — no reserve leaks', () => {
  ...
  assert.ok(
    spec.state.decks.stage1.every((x) => x === null),
    'decks still blanked for spectator',
  );
});
...
test('FIX2: a client-supplied seed is ignored (server mints its own RNG)', () => {
  ...
  assert.notStrictEqual(
    got,
    ifHonored,
    'server must not honor the client seed (would leak deck order)',
  );
});
...
test('FIX3: rebind silently restores a seat by token (hibernation wake, no message storm)', () => {
  ...
  room2.restore(snap);
  room2.rebind('cA', 'tA');
  room2.rebind('cB', 'tB'); // quiet re-attach
  assert.strictEqual((inbox2['cA'] || []).length, 0, 'rebind emits nothing');
  room2.onMessage('cB', { t: 'action', seq: 1, action: TAKE });
  assert.ok(
    (inbox2['cB'] || []).some((m) => m.t === 'reject'),
    'seat mapping restored: cB blocked on cA turn',
  );
  room2.onMessage('cA', { t: 'action', seq: 1, action: TAKE });
  assert.ok(
    (inbox2['cA'] || []).some((m) => m.t === 'state'),
    'cA can act after rebind',
  );
});
...
test('FIX3: a lobby that hibernated BEFORE start is not bricked (host can still start)', () => {
  ...
  assert.ok(
    (inbox2['cA'] || []).some((m) => m.t === 'state'),
    'host could start after pre-start hibernation',
  );
...
test('takeover: host-only + must wait the timeout, then plays the active seat & advances', () => {
  ...
  assert.ok(
    last('cA', 'reject') && /超时/.test(last('cA', 'reject').reason),
    'rejected before timeout',
  );
